...
(function(){
  ...
  function renderPay(){
    ...
    orderInfo.append(orderTitle, orderChannel);
    const amount = document.createElement('div');
    amount.className = 'amount-due';
    const amountLabel = document.createElement('small');
    amountLabel.textContent = 'Amount due';
    const amountValue = document.createElement('strong');
    amountValue.textContent = `${amountDue} GHS`;
    amount.append(amountLabel, amountValue);
    identity.append(orderInfo, amount);
    const totals = document.createElement('div');
    totals.className = 'payment-totals';
    totals.setAttribute('aria-label', 'Payment total');
    [['Subtotal', `${c.subtotal} GHS`], ['Discount', `-${discount} GHS`]].forEach(([label, value])=>{
    ...
    if(isPrepaidPlatform(s)){
      ...
      [
        {label:'Cash', method:'cash', provider:'', disabled: paymentDisabled || (isWhatsapp(s) && s.fulfilment === 'delivery')},
        {label:'Telecel MoMo', method:'momo', provider:'telecel', disabled: paymentDisabled},
        {label:'MTN MoMo', method:'momo', provider:'mtn', disabled: paymentDisabled}
      ].forEach(option=>{
        const button = document.createElement('button');
        ...
        button.onclick = ()=> requestSlotPayment(active, option.method, option.provider);
        methods.appendChild(button);
      });
      panel.appendChild(methods);
    }else{
      const changeButton = document.createElement('button');
      changeButton.className = 'x change-payment-action';
      changeButton.type = 'button';
      changeButton.textContent = 'Change payment';
      if(canReversePayment) changeButton.onclick = ()=>requestSlotPayment(active, 'unpaid');
